...
//		String content = "前日爆出《大话西游终结篇》演员阵容上，由韩庚、唐嫣顶替星爷、朱茵出演至尊宝、紫霞仙子，此外，曝光其他演员还有吴京、何炅、林志玲、陈伟霆、TFBOYS，你没有看错，就是国内亲妈粉最多的SB组合（Super Boys）TFBOYS，这阵容里有几个是真演员啊，妥妥奔着票房去毁经典，这哪是大话，简直就是笑话。 \u200B\u200B\u200B";
	...
	public void testFileProcess(){
		double flag = NlpirMethod.NLPIR_FileProcess("src/ictclas/test/source.txt", "test/result.txt", 1);
	...
	public void testFileKeyWords(){
		String result = NlpirMethod.NLPIR_GetFileKeyWords("src/ictclas/test/测试.txt", 10, false);
		System.out.println(result);
	}
	...
	public void testFileNewWords(){
		String result = NlpirMethod.NLPIR_GetFileNewWords("src/ictclas/test/source.txt", 100, true);
		System.out.println(result);
	}
...
//		int flag1 = NlpirMethod.NLPIR_AddUserWord("俄罗斯卫星网");
		System.out.println(flag1);

		int flag2 = NlpirMethod.NLPIR_SaveTheUsrDic(); // 保存到文件，不然会随内存一起丢失
		System.out.println(flag2);
	...
	public void testImportUserDict(){
		int flag = NlpirMethod.NLPIR_ImportUserDict("src/ictclas/dict/user_dict.txt", true);
		System.out.println(flag);
	}
	...
	public void testImportKeyBlack(){
		int flag = NlpirMethod.NLPIR_ImportKeyBlackList("src/ictclas/dict/black_dict.txt");
		System.out.println(flag);
	}
	...
	public void testFileWordFreq(){
		String result = NlpirMethod.NLPIR_FileWordFreqStat("src/ictclas/test/source.txt");
		System.out.println(result);
	}
